...
assigned: 2026-02-25 09:00:00.00-08:00
...
**Video 1 (19 min):** [3Blue1Brown: But what is a neural network?](https://youtu.be/aircAruvnKk?si=KZt2AsbD7URc58-L)
...
- ```utility``` contains the activation functions (```identity```, ```ReLU```, ```sigmoid```) and their derivatives, as well as other miscellaneous functions used by the neural network.
...
#### Connecting the Videos to the Code

Before continuing to Step 5, watch this second video. It covers what training means — how a network learns by adjusting its weights — which is exactly what `contribute()` and `update()` implement.

**Video 2 (20 min)** — What training means: how a network learns by adjusting its weights:
[3Blue1Brown: Gradient descent, how neural networks learn](https://youtu.be/IHZwWFHWa-w?si=_e4wkFW624ari842)

After watching these videos, here is how the key visuals map to this assignment:
...
**The loss function** measures how wrong a prediction was. For a single prediction $p$ and ground truth $y$, a simple loss is:

$$L = (y - p)^2$$

Training is the process of adjusting weights and biases to make $L$ smaller over many examples. Each call to `contribute()` followed by `update()` is one step downhill on that curve.
...
When a neural network makes a prediction, there is a sense of "how bad" the prediction was — this is exactly what the loss function from the "Connecting the Videos to the Code" section measures. The neural network has a mechanism for finding this out, which we've abstracted behind the visit functions. Curious students are encouraged to check the [Gentle Introduction to Neural Networks]({{site.baseurl}}/pa/pa03-tutorial/) tutorial for more details.
...
Think of the loss function as a hilly landscape where the height at any point represents how wrong the network's prediction is. Every weight and bias is a dimension of that landscape, and the goal is to roll the ball downhill to a lower point. But to know which direction is "downhill" for each weight, the network needs to know how much each weight contributed to the current height (the error). That is what back-propagation computes. Starting at the output node — where the loss is measured directly — each node calculates its share of the error and passes a scaled portion back to its incoming connections. This continues layer by layer until every weight and bias has received its nudge, telling `update()` exactly how far and in which direction to move each one downhill. 
...
<div style="width:100%;height:700px;margin:1rem 0;border-radius:8px;overflow:hidden;border:1px solid #dee2e6;">
  <iframe src="{{site.baseurl}}/assets/backprop_viz.html"
...
[Open in full screen]({{site.baseurl}}/assets/backprop_viz.html){:target="_blank"}
